...
void handle_lsbi_extraction(Args *args)
{
        ...
        printf("BMP Analysis:\n");
        printf("Image dimensions: %d x %d\n", carrier->info_header->width,
               carrier->info_header->height);
        ...
        printf("Final pattern map: %d%d%d%d\n", (pattern_map >> 3) & 1, (pattern_map >> 2) & 1,
               (pattern_map >> 1) & 1, pattern_map & 1);
        ...
        for (int i = 0; i < 32; i++) {
                ...
                printf("Bit %2d: Byte=%02X Pattern=%d LSB=%d (after inversion=%d)\n", i,
                       current_byte, pattern, current_byte & 0x01, bit);
...
char *extract_payload_extension(uint8_t *payload, uint32_t message_size, uint8_t step)
{
        ...
        for (i = 0; last_read != '\0' && i < MAX_EXTENSION_LENGTH - 1; i++) {
                last_read =
                        extract_char(payload, base_index + i * sizeof(char) * 8 / step, step,
