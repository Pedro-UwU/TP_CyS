...
static uint8_t check_pixel_changes(unsigned char *original, unsigned char *modified, size_t length, uint8_t pattern) {
...
void handle_lsbi(Args *args) {
    ...
    for(uint8_t pattern = 0; pattern < 4; pattern++) {
        if(check_pixel_changes(original_payload, bmp->payload, bmp->info_header->sizeImage, pattern)) {
