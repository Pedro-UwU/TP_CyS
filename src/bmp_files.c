...
void describe_bmp(const BmpFile *bmp)
{
        ...
        printf("  Resolution: %u x %u pixels/meter\n", bmp->info_header->xPelsPerMeter,
               bmp->info_header->yPelsPerMeter);
...
void save_bmp(BmpFile *bmp, char *path)
{
        ...
        if (fwrite((unsigned char *)bmp->header, sizeof(BmpHeader), 1, output_file) != 1) {
        ...
        if (fwrite(bmp->payload, 1, bmp->info_header->sizeImage, output_file) !=
            bmp->info_header->sizeImage) {
